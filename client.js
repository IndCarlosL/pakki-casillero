...
const clientApp = {
    ...
    renderDashboard: function() {
        ...
        // Pakki Express — siempre visible
        const expressBlock = document.getElementById('cl-express-address');
        if (expressBlock) {
            expressBlock.innerHTML = `<strong>${shortName} [${loggedUser.lockerCode}]</strong>\n8524 NW 61st ST, Suite 105\nMiami, FL 33166`;
        }

        // Pakki Business — solo para corporativos
        const businessCard = document.getElementById('cl-business-card');
        const businessBlock = document.getElementById('cl-business-address');
        if (businessCard) businessCard.style.display = esCorp ? 'flex' : 'none';
        if (businessBlock && esCorp) {
            businessBlock.innerHTML = `<strong>${shortName} [${loggedUser.lockerCode}]</strong>\n7128 NW 52nd ST\nMiami, FL 33166`;
        }
